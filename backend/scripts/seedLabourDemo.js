...
async function run() {
    ...
    if (REMOVE) {
        ...
        // Attendance rows reference labour, so they go first: leaving them
        // would orphan rows that every report then has to defend against.
        ...
        console.log(`\nremoved: ${attendance.deletedCount} attendance row(s), ` +
            `${labour.length} labour, ${sites.length} site(s), ${users.length} user(s).`);
        await mongoose.disconnect();
        return;
    }
